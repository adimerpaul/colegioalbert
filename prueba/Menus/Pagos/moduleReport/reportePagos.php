...
$res=$_GET['var'];
 ...
 $i=mysqli_fetch_array(mysqli_query($conectador,"select * from pago_estudiantes where Password ='$res';"));
?>
<html>
<head>
...
<table class="table table-info mes"style='float: left; width : 48%'>
  ...
  <tbody>
    ...
    <?php $rest=mysqli_query($conectador,"select mes from pago_estudiantes where Password ='$res';");
...
<table class="table table-info">
  <thead  class="table-warning">
    <tr>
      ...
      <?php $restO=mysqli_query($conectador,"select SUM(importe_recibido) from pago_estudiantes where Password ='$res';");
     while($h=mysqli_fetch_array($restO)) {?>
...
<table class="table table-info">
  <thead  class="table-warning">
    <tr>
     ...
     <?php /* $descuentos=mysqli_fetch_array(mysqli_query($conectador,"select sum(descuento) from pago_estudiantes where rude_estu ='$res'"));*/ ?>
         <th>Proximas fechas de pago</th> 
...
<table class="table table-info">
  <thead  class="table-warning">
    <tr>
      ...
      <?php	$prim_uno=mysqli_query($conectador,"select MAX(importe_mensual) from pago_estudiantes where Password= '$res' LIMIT 1" );
         foreach (mysqli_fetch_array($prim_uno) as $rowPrim );
         ?> 
   
      
      
      <?php
      $descuentos=mysqli_query($conectador,"select sum(descuento) from pago_estudiantes where Password= '$res'");
      foreach (mysqli_fetch_array($descuentos) as $rowPagos );
      


      $pendienteTotal=$rowPrim*10;

      ?>
      <th class="table-info" scope="col"><?php echo "Cuota mensual: ".$rowPrim.' Bs';
            echo'<br>';
          echo 'Descuento: '.$rowPagos;
          echo'<br>';
          echo 'Pago de gestión: '.$pendienteTotal=$rowPrim*10;
          $num_pagos=mysqli_query($conectador,"select sum(importe_mensual) from pago_estudiantes where Password= '$res'");
foreach (mysqli_fetch_array($num_pagos) as $rownumPagos);

          echo'<br>';
          echo 'Pagos pendientes: '.$pagoFaltante=$pendienteTotal-$rownumPagos;
