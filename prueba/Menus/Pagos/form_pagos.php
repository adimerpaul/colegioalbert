...
?>
<div class="container">
<form  class="form-horizontal" method="post"  name="form1" action="pro_reg_cobro.php?rude=<?php echo $dato;?>&user=<?php echo $dato2;?>"  onSubmit="onEnviar()">
      ...
      <tr>
        ...
        <td width="37%">Gestion:&nbsp;
          ...
          <select name="year" id="year">
           <option value="<?php echo $años1=date('Y');?>">
            <?php 
            $años1=date('Y');
            print ($años1); 
            ?>
            </option>
            <option value="<?php echo $años2=(date('Y')+1);?>">
            <?php 
            $años2=(date('Y')+1);
            print ($años2); 
            ?>
...
document.write(diasSemana[f.getDay()] + " " + f.getDate() + " de " + meses[f.getMonth()] + " del " + f.getFullYear());</script><!--fin del script--></h4></td>
      ...
      <tr>
        <td height="60">Codigo estudiante</td>
    ...
    <!--2DA tabla donde se agregan cobros-->
      <table class="table" border="0" style="border-color:#eca9a7;">
        <tr>
          ...
          <td width="23%" bgcolor="#FFFF66"  style="text-align: center;"><font face="georgia">Mensualidad</font></td>
          <td width="19%" bgcolor="#FFFF66" style="text-align: center;"><font face="georgia">Importe mensual</font></td>
        </tr>
        <tr>
          <td align="center"><input class="form-control" type="number" name="cantidad" id="cantidad" value="1" style=" text-align:center; width:50px; height:auto" readonly>
                </td>
          <td align="center">
                  <?php
                  if(isset($posMaxdato)){
                    $posMaxdato=$posMaxdato+1;
                  } 
                  else{
                   $posMaxdato=0; 
                  }  
                   if($posMaxdato<=9)
                   {
                    echo "<select  class='form-control' name='mes' id='mes'>";
                     while($posMaxdato<=9)
                      {
                        echo "<option>$mensualidad[$posMaxdato]</option>";
                        $posMaxdato++;
                      }
                      echo "</select>";
                   }
                  ...
                  {
                    echo "SIN MENSUALIDADES PENDIENTES";
                  }
                   ?>
          </td>
          <td align="center">
            <?php
              if($i2[3]==1){
                ?>
                <input class="form-control" type="number" style="text-align:center; width:100px; height:auto" size="10" name="im" id="im" value="<?php echo $ul_imp[0]?>">
              <?php
              }
              else{
                ?>
                <input class="form-control" type="number" style="text-align:center; width:100px; height:auto" size="10" name="im" id="im" value="<?php echo $ul_imp[0]?>" readonly>
              <?php
              }
            ?>
    ...
    <!--3RA tabla donde se acumulan los cobros-->
      <table id="tabla_acumuladora" name="tabla_acumuladora" width="300" class="table table-hover table-striped" border="0" style="border-color:#eca9a7;">
        <tr>
          <th width="19%" bgcolor="#FFFF66" style="text-align: center;"><font face="georgia">Cantidad</font></th> 
          <th width="23%" bgcolor="#FFFF66"  style="text-align: center;"><font face="georgia">Mensualidad</font></th>
          <th width="19%" bgcolor="#FFFF66" style="text-align: center;"><font face="georgia">Importe mensual</font></th>
      ...
      <h2 align="center"><font Color='RED'>Extracto de pensiones pagadas</font></h2>
      ...
      <table class="table table-hover" width="873" align="center">
        <tr class="amarillo_reporte">
          ...
          <td class="danger">Mes cancelado</td>
        ...
        $q=mysqli_query($conectador,"select * from pago_estudiantes where rude_estu='$dato';");
  ...
  <script type="text/javascript">      
            ...
            var importe_mensual;
                function ShowSelected() 
                {
                  ...
                  var importe_mensual=document.getElementById("im").value;
                  var total=cant*importe_mensual;
                ...
                importe_mensual=document.getElementById('im').value
                
                
                var fila='<tr id="fila'+c+'">'
               +'<td align="center">1</td>'
               +'<td><input class="form-control" type="text" name="meses[]" value="'+m+'" style="background-color: white;" readonly></td>'
               +'<td><input style="text-align:center;" class="form-control" type="text" name="importe_m[]" value="'+importe_mensual+'" style="background-color: white;" readonly></td>'
